...
let ALI_PAY_SETTINGS = {
  ...
  APP_PRIVATE_KEY_PATH: path.join(
    __dirname,
    "pem",
    "remind",
    "sandbox",
...
class AliPayHelper {
  ...
  buildParams(subject, outTradeNo, totalAmount) {
    ...
    params.set(
      "biz_content",
      this._buildBizContent(subject, outTradeNo, totalAmount)
    );
  ...
  _buildSign(paramsMap) {
    ...
    let privateKey = fs.readFileSync(
      this.accountSettings.APP_PRIVATE_KEY_PATH,
      "utf8"
    );
  ...
  verifySign(params) {
    try {
      ...
      let paramsList = [...paramsMap].filter(
        ([k1, v1]) => k1 !== "sign" && k1 !== "sign_type" && v1
      );
      //2.按照字符的键值ASCII码递增排序
      paramsList.sort();
      //3.组合成“参数=参数值”的格式，并且把这些参数用&字符连接起来
      ...
      let publicKey = fs.readFileSync(
        this.accountSettings.ALI_PUBLIC_KEY_PATH,
        "utf8"
      );
